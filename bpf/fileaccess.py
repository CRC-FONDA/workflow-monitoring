...
group.add_argument(
    "--nofilter",
    action="store_true",
    help="apply no filter (warning: may lead to high output and degradation of overall performance; may want to use --time as well then)",
)
group.add_argument(
    "-i", "--id", type=str, help="filter by comma separated list of PIDs"
)
group.add_argument(
    "-p",
    "--program",
    type=str,
    help="commandline to call program that is going to be logged",
)
group.add_argument(
    "-u",
    "--user",
    type=str,
    help="filter by comma separated list of UIDs or user names",
)
group.add_argument(
    "-g",
    "--group",
    type=str,
    help="filter by comma separated list of GIDs or group names",
)
parser.add_argument(
    "-P",
    "--path",
    type=str,
    help="filter by comma separated list of paths (provided by script, not BPF)",
)
parser.add_argument(
    "-o",
    "--operations",
    type=str,
    help="specifies which operations to log (default: OCRWU)",
    default="OCRWU",
)
parser.add_argument(
    "-c", "--includechildren", action="store_true", help="also include child processes"
)
parser.add_argument(
    "-O",
    "--output",
...
parser.add_argument(
    ...
    "--time",
    type=int,
    help="number of seconds to run the trace, 0 for unlimited (default: 0)",
    default=0,
)
parser.add_argument(
    "--depth",
    type=int,
    help="depth of loop unrolling, equals supported path depth (default: 10)",
    default=10,
)
parser.add_argument(
    "--bufsize",
    type=int,
    help="size of the path buffer, maximum supported size per path part (default: 64)",
    default=64,
)
parser.add_argument(
    "--delimiter",
    type=str,
    help="delimiter of output fields (default comma)",
    default=",",
)
parser.add_argument(
    "--pathoutput",
    type=int,
    help="number of characters for path output (default 240)",
    default=240,
)
parser.add_argument(
    "--evbufmain",
...
parser.add_argument(
    "--evbufpath",
    type=int,
    help="page count for path event buffer (default: 8)",
    default=8,
)
parser.add_argument(
    "--perfbuf",
...
parser.add_argument(
    "--usermodepathfilter",
    action="store_true",
    help="filter by path (-P/--path) in usermode (Python) instead of BPF code (might be needed for older kernels)",
)
parser.add_argument(
    "--printonly",
    action="store_true",
...
bpf_text = bpf_text.replace(
    "FILTER_BY_PATH",
    "1" if args.path is not None and not args.usermodepathfilter else "0",
)

bpf_text = bpf_text.replace(
    "INCLUDE_CHILD_PROCESSES", "1" if args.includechildren else "0"
)
bpf_text = bpf_text.replace(
    "LOG_DELETES", "1" if args.operations.find("U") >= 0 else "0"
)
...
if args.operations.find("W") >= 0:
    ...
    try:  # on older kernels those functions don't exist and attaching will raise an exception; ignoring it is fine
        ...
        b.attach_kprobe(
            event="vfs_iocb_iter_write", fn_name="probe__vfs_iocb_iter_write"
        )
    except:
        pass
...
if args.id is not None:
    ...
    try:
        b["log_pids"].items_update_batch(
            (ctypes.c_uint32 * len(pids))(*pids),
            (ctypes.c_uint32 * len(pids))(*([1] * len(pids))),
        )
...
if args.user is not None:
    ...
    uids = [
        (int(uid) if uid.isnumeric() else getpwnam(uid).pw_uid)
        for uid in args.user.split(",")
    ]
    try:
        b["log_uids"].items_update_batch(
            (ctypes.c_uint32 * len(uids))(*uids),
            (ctypes.c_uint32 * len(uids))(*([1] * len(uids))),
        )
    except:  # items_update_batch only supported since kernel 5.6, else update element by element
        for u in uids:
...
if args.group is not None:
    ...
    gids = [
        (int(gid) if gid.isnumeric() else getgrnam(gid).gr_gid)
        for gid in args.group.split(",")
    ]
    try:
        b["log_gids"].items_update_batch(
            (ctypes.c_uint32 * len(gids))(*gids),
            (ctypes.c_uint32 * len(gids))(*([1] * len(gids))),
        )
    except:  # items_update_batch only supported since kernel 5.6, else update element by element
        for g in gids:
...
def output_event(event):
    ...
    if chr(event.type) == "O":
        if event.handle_uid in open_pathes:
            path = (
                open_pathes[event.handle_uid].pathes["F"]
                + ":"
                + open_pathes[event.handle_uid].pathes["M"]
                + ":"
                + open_pathes[event.handle_uid].pathes["S"]
            )
        ...
        path = (
            delete_pathes[event.inode_uid].pathes["F"]
            + ":"
            + delete_pathes[event.inode_uid].pathes["M"]
            + ":"
            + delete_pathes[event.inode_uid].pathes["S"]
        )
    ...
    if args.operations.find(chr(event.type)) >= 0 and (
        not args.usermodepathfilter
        or (args.path is None or event.handle_uid in allowed_handles or unlink_ok)
    ):
...
def handle_path(cpu, data, size):
    ...
    if event.last == 0:
        if event.filename != b"/":
            ...
            pathes[event.uid].pathes[chr(event.path_type)] = (
                "/"
...
def handle_main(cpu, data, size):
    ...
    if chr(event.type) == "U" and (
        not event.inode_uid in delete_pathes
        or delete_pathes[event.inode_uid].ready == 0
    ):
        saved_delete_event[event.inode_uid] = [event]
    elif chr(event.type) == "O" and (
        not event.inode_uid in open_pathes or open_pathes[event.inode_uid].ready == 0
    ):
